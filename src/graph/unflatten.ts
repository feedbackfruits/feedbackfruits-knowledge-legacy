...
function mergeObjects<T>(result: T, other: T, cache: Array<T> = []): T {
  return Object.keys(result).reduce<T>((memo, key) => {
    ...
    if (value instanceof Array || otherValue instanceof Array) {
      ...
      throw new Error('Values are native and do not match');
    }
    else {
      memo[key] = value;
    }
...
function mergeResultsWithResult<T>(results: Array<T>, result: T): Array<T> {
  ...
  else if (results.length === 1) return [ mergeObjects(results[0], result) ];
  else return results.reduce((memo, value) => {
    return mergeResultsWithResult(memo, value);
    // if (!(ID in value)) {
    //   debugger;
    //   return
    // }
...
function processResults<T>(results: Array<T>) {
